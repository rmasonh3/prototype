...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
    ...
    @Query(
        value = "select distinct employee from Employee employee left join fetch employee.user left join fetch employee.workrequest",
    ...
    @Query("select distinct employee from Employee employee left join fetch employee.user left join fetch employee.workrequest")
    List<Employee> findAllWithToOneRelationships();

    @Query(
        "select employee from Employee employee left join fetch employee.user left join fetch employee.workrequest where employee.id =:id"
    )
